...
var Influx = require('influx'),
    ...
    impl = {
        ...
        addRunningMeasurements: function (instance, influxReporter, testReport) {
            var stats,
                fields = {},
                time = new Date().getTime().toString() + (Math.floor(Math.random() * 900000) + 100000);
            ...
            return metric.getMetrics()
                .then(function(result) {
                    ...
                    fields = {
                        time: time,
                        [instance.config[constants.CONFIG_CPU_MEASUREMENT_NAME]]: stats.cpu,
                        [instance.config[constants.CONFIG_MEMORY_MEASUREMENT_NAME]]: stats.memory,
                        [instance.config[constants.CONFIG_AVOIDED_SCENARIOS_MEASUREMENT_NAME]]: testReport._scenariosAvoided,
                        [instance.config[constants.CONFIG_PENDING_REQUESTS_MEASUREMENT_NAME]]: testReport._pendingRequests
                    };

                    influxReporter.writePoint('artillery_stats', fields, instance.config[constants.CONFIG_STATIC_TAGS], function (err) {
                        if (err) {
        ...
        reportResults: function (instance, influxReporter, testReport) {
            ...
            while (samples < testReport.latencies.length) {
                ...
                tags.response = sample[constants.STATUS_CODE];
                if (sample[constants.PATH]) {
                    tags.path = sample[constants.PATH];
                }
                Object.assign(tags, instance.config[constants.CONFIG_STATIC_TAGS]);
                ...
                if (JSON.parse(JSON.stringify(instance.config[constants.CONFIG_INFLUX].matches)) === constants.CONFIG_SHOW_MATCHES) {
                    points.push([{
                        ...
                        time: sample[constants.TIMESTAMP].toString() + (Math.floor(Math.random() * 900000) + 100000),
            ...
            impl.addRunningMeasurements(instance, influxReporter, testReport);
        },
        reportErrors: function (instance, influxReporter, testReport) {
            ...
            Object.getOwnPropertyNames(testReport.errors).forEach(function (propertyName) {
                ...
                points.push([{
                    time: Date.now().toString() + (Math.floor(Math.random() * 900000) + 100000),
                    value: testReport.errors[propertyName]
                }, Object.assign({ error: propertyName }, instance.config[constants.CONFIG_STATIC_TAGS])]);
            });
