...
var Influx = require('influx'),
    ...
    impl = {
        ...
        addRunningMeasurements: function (instance, influxReporter, testReport, tags) {
            var stats,
                fields = {},
                time = new Date();
            ...
            return metric.getMetrics()
                .then(function(result) {
                    ...
                    influxReporter.writePoint(instance.config[constants.CONFIG_CPU_MEASUREMENT_NAME], fields, tags);
                    fields = {
                        time: time,
                        value: stats.memory
                    };

                    influxReporter.writePoint(instance.config[constants.CONFIG_MEMORY_MEASUREMENT_NAME], fields, tags);
                    fields = {
                        time: time,
                        value: testReport._scenariosAvoided
                    };

                    influxReporter.writePoint(instance.config[constants.CONFIG_AVOIDED_SCENARIOS_MEASUREMENT_NAME], fields, tags);
                    fields = {
                        time: time,
                        value: testReport._pendingRequests
                    };

                    influxReporter.writePoint(instance.config[constants.CONFIG_PENDING_REQUESTS_MEASUREMENT_NAME], fields, tags);
                });
        },
        ...
        reportResults: function (instance, influxReporter, testReport) {
            ...
            impl.addRunningMeasurements(instance, influxReporter, testReport, tags);
        },
